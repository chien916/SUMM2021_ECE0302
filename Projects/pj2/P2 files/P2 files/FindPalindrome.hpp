...
private:
    ...
    //custom private containers
    std::vector<std::string> objVectorString;//stores all added strings of words
    std::vector<std::vector<std::string>> Palindromes;//stores all possible Palindromes


    //custom private functions
    bool scanString(const std::string &stringToScan);
    std::string stringVectorToString(const std::vector<std::string> &stringVector,const int control);
    void recomputePalindrome();
