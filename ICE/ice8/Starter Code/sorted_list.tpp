...
void SortedList<T, L>::insert(const T &item) {
    int positionToInsert = plist.getLength();
    for (int i = 0; i < plist.getLength(); i++) {//find proper position to insert
        if (item <= plist.getEntry(i)) {//compare value (valid for most primitive data types)
            positionToInsert = i;
            break;
        }
    }
    plist.insert(positionToInsert, item);
}
...
long int SortedList<T, L>::getPosition(const T &newValue) {
    ...
    for (int i = 0; i < plist.getLength(); i++) {//find proper position to insert
        if (newValue == plist.getEntry(i)) {//compare value (valid for most primitive data types)
            positionToInsert = i;
            break;
        }
    }
